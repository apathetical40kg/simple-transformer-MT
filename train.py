...
def get_model(config, vocab_src_len, vocab_tgt_len):
    model = build_transformer(vocab_src_len, vocab_tgt_len, config['seq_len'], config['seq_len'], config['embedding_dim'])
    return model
...
def train_model(config):
    ...
    if config['preload']:
        ...
        print(f'Preload {model_filename}')
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            model.train()
            encoder_input = batch['encoder_input'].to(device) # [B, L]
            decoder_input = batch['decoder_input'].to(device)
            encoder_mask = batch['encoder_mask'].to(device)
            deocoder_mask = batch['decoder_mask'].to(device)

            encoder_output = model.encode(encoder_input, encoder_mask) # [B, L, E]
            decoder_output = model.decode(encoder_output, encoder_mask, decoder_input, deocoder_mask) # [B, L, E]
            projection = model.projection(decoder_output) # [B, L, V]

            label = batch['label'].to(device) # [B, L]

            loss = loss_fn(projection.view(-1, tokenizer_tgt.get_vocab_size()), label.view(-1)) # [B * L, V], [B * L]
